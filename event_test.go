...
func benchmarkParse(b *testing.B, eventID, eventJSON string) {
	// run the Unparse function b.N times
	for n := 0; n < b.N; n++ {
		//if _, err := NewEventFromUntrustedJSON([]byte(eventJSON), RoomVersionV1); err != nil {
		if _, err := NewEventFromTrustedJSONWithEventID(eventID, []byte(eventJSON), false, RoomVersionV1); err != nil {
