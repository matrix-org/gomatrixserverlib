...
func TestHandleSendJoin(t *testing.T) {
	...
	ebExistingMemberBanned := createEventBuilder(userID.String(), validRoom.String(), &stateKey, spec.RawJSON(`{"membership":"ban"}`))
	existingMemberBannedEvent, err := ebExistingMemberBanned.Build(time.Now(), userID.Domain(), keyID, sk)
	assert.Nil(t, err)
	...
	tests := map[string]struct {
		...
		"sender_does_not_match_request_origin": {
			input: HandleSendJoinInput{
				...
				LocalServerName: localServer,
				StateQuerier:    &TestRestrictedRoomJoinQuerier{roomExists: true, serverInRoom: map[string]bool{validRoom.String(): true}},
				KeyID:           keyID,
				PrivateKey:      sk,
				Verifier:        verifier,
			},
		...
		"state_event_lookup_failure": {
			input: HandleSendJoinInput{
				...
				RequestOrigin:   remoteServer,
				LocalServerName: localServer,
				StateQuerier:    &TestRestrictedRoomJoinQuerier{roomExists: true, stateEventErr: true, serverInRoom: map[string]bool{validRoom.String(): true}},
				KeyID:           keyID,
				PrivateKey:      sk,
				Verifier:        verifier,
			},
		...
		"existing_member_banned": {
			input: HandleSendJoinInput{
				...
				RequestOrigin:   remoteServer,
				LocalServerName: localServer,
				StateQuerier:    &TestRestrictedRoomJoinQuerier{roomExists: true, memberEvent: existingMemberBannedEvent, serverInRoom: map[string]bool{validRoom.String(): true}},
				KeyID:           keyID,
				PrivateKey:      sk,
				Verifier:        verifier,
			},
		...
		"existing_member_allowed": {
			input: HandleSendJoinInput{
				...
				RequestOrigin:   remoteServer,
				LocalServerName: localServer,
				StateQuerier:    &TestRestrictedRoomJoinQuerier{roomExists: true, memberEvent: joinEvent, serverInRoom: map[string]bool{validRoom.String(): true}},
				KeyID:           keyID,
				PrivateKey:      sk,
				Verifier:        verifier,
			},
