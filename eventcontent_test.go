...
func TestStrictPowerLevelContent(t *testing.T) {
	...
	goodEvent, err := newEventFromTrustedJSONV2([]byte(eventJSON), false, MustGetRoomVersion(RoomVersionV7))
	if err != nil {
		t.Fatal(err)
	}
	badEvent, err := newEventFromTrustedJSONV2([]byte(eventJSON), false, MustGetRoomVersion("org.matrix.msc3667"))
	if err != nil {
		t.Fatal(err)
	}
