...
func TestMarshalInviteV2Request(t *testing.T) {
	...
	if err := json.ConfigCompatibleWithStandardLibrary.Unmarshal([]byte(TestInviteV2ExampleEvent), &output); err != nil {
		t.Fatal(err)
	}
	...
	j, err := json.ConfigCompatibleWithStandardLibrary.Marshal(inviteReq)
	if err != nil {
		t.Fatal(err)
	}
...
func TestStrippedState(t *testing.T) {
	...
	j, err := json.ConfigCompatibleWithStandardLibrary.Marshal(stripped)
	if err != nil {
		t.Fatal(err)
	}
