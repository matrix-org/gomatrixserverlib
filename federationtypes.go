...
func (r *RespMissingEvents) UnmarshalJSON(data []byte) error {
	...
	for _, raw := range intermediate.Events {
		event, err := NewEventFromUntrustedJSON([]byte(raw), r.roomVersion, r.verifier)
		if err != nil {
			return err
		}
...
func (r *RespPeek) UnmarshalJSON(data []byte) error {
	...
	latestEvent, err := NewEventFromUntrustedJSON([]byte(intermediate.LatestEvent), r.RoomVersion, r.verifier)
	if err != nil {
		return fmt.Errorf("RespPeek UnmarshalJSON(StateEvent): %w", err)
	}
...
func (r RespInvite) MarshalJSON() ([]byte, error) {
	...
	return json.Marshal([]interface{}{200, respInviteFields{Event: r.Event}})
}
...
func (r *RespInvite) UnmarshalJSON(data []byte) error {
	...
	if jr := gjson.GetBytes(tuple[1], "event"); jr.Exists() {
		event, err := NewEventFromUntrustedJSON([]byte(jr.Raw), RoomVersionV1, r.verifier)
		if err != nil {
			return err
		}
		r.Event = event
...
func (r *RespInviteV2) UnmarshalJSON(data []byte) error {
	...
	event, err := NewEventFromUntrustedJSON([]byte(intermediate.Event), r.roomVersion, r.verifier)
	if err != nil {
		return err
	}
	r.Event = event
