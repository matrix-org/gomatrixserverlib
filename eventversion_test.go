...
func TestEventIDForRoomVersionV3(t *testing.T) {
	...
	event, err := newEventFromTrustedJSON([]byte(initialEventJSON), false, MustGetRoomVersion(RoomVersionV3))
	if err != nil {
		t.Error(err)
	}
...
func TestEventIDForRoomVersionV4(t *testing.T) {
	...
	event, err := newEventFromTrustedJSON([]byte(initialEventJSON), false, MustGetRoomVersion(RoomVersionV4))
	if err != nil {
		t.Error(err)
	}
