...
func TestUnmarshalMarshalHeaderedEvent(t *testing.T) {
	...
	event, err := NewEventFromTrustedJSON([]byte(input), true, RoomVersionV1)
	if err != nil {
		t.Fatalf("failed to create event: %v", err)
	}
	output := event.Headered(RoomVersionV1)

	if j, err := json.Marshal(output); err == nil {
		if string(j) != input {
			t.Logf("got %d: %s", len(string(j)), string(j))
			t.Logf("expected %d: %s", len(input), input)
